...
export default function BOQManagement() {
  ...
  const { data: bills, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<Bill[]>(`/boq/bills?project_id=${projectId}`);
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div key={bill.id} className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <div className="text-lg font-medium">{bill.section_count}</div>
                </div>
                <div>
              ...
              <div className="flex space-x-2">
                <button 
                  ...
                  onClick={() => window.location.href = `/projects/${projectId}/boq/bills/${bill.id}`}
                >
        ...
        {bills?.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Create your first bill to start building the BOQ.</p>
          </div>
        )}
      </div>
    </div>
